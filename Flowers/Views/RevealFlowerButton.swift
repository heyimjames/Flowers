...
struct RevealFlowerButton: View {
    ...
    private func createConfetti() {
        confettiPieces = (0..<80).map { index in
            let angle = Double(index) * (360.0 / 80.0) * .pi / 180.0
            let baseVelocity = CGFloat.random(in: 150...350)
            let isGreen = index % 3 != 0 // More green than white for better effect
            
            return ConfettiPiece(
                id: UUID(),
                color: isGreen ? Color.flowerPrimary : Color.white,
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: piece.size / 4)
            ...
            .onAppear {
                ...
                let destX = cos(piece.angle) * piece.velocity
                let destY = sin(piece.angle) * piece.velocity
                ...
                withAnimation(.easeOut(duration: 1.8)) {
                    ...
                    opacity = 0
                    scale = 0.2
                    rotation = Double.random(in: 360...720) // Spin while flying
                }
            }
    }
} 